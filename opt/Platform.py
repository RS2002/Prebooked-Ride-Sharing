...
def reward_func_generator(beta_list, threshold):
    def reward(time_add,time_out,pickup_time,direct_distance):
        ...
        profit = beta_list[0] + beta_list[1] * direct_distance / 1000
        return r, profit
    return reward
...
class Platform():
    ...
    def obs(self, observe, order,  current_order_state, current_order_num, time_threshold, reward_func, reward_parameter_list, current_time, pre=True):
        if pre:
            results = Parallel(n_jobs=self.njobs)(
                delayed(obs_pre)(observe[i], order[j], current_order_state[i], current_order_num[i], time_threshold, reward_func, reward_parameter_list, current_time)
                for i in range(observe.shape[0])
                for j in range(order.shape[0])
            )
        else:
            results = Parallel(n_jobs=self.njobs)(
                delayed(obs)(observe[i], order[j], current_order_state[i], current_order_num[i], time_threshold, reward_func, reward_parameter_list, current_time)
                for i in range(observe.shape[0])
                for j in range(order.shape[0])
            )

        results = np.array(results)
        results = results.reshape(1000, -1)
        return results
...
6 -- pickup reward
7 -- potential overtime assignment punishment
8 -- already overtime punishment
'''
...
def excute(observe_pre, order_pre, observe, current_order_state, current_order_num, assignment, new_orders_state, time_threshold, reward_func, reward_parameter_list, current_time):
    ...
    if order_pre is not None: # has pre-booked order
        ...
        if observe[2]==0 or (observe[2:8] == observe_pre[2:8]).all():
            log["swap"] = 0
        else:
            log["swap"] = 1
        ...
        if worker_type == 1: # if the worker is not available (current_order_num>=1)
            if order_pre_type == 0: # if the pre-booked order allows pooling
                if current_order_state[0,4] == 0: # if the unfinished orders allow pooling
                    if current_order_num == current_order_state.shape[0]: # need to wait until having a new available seat
                        ...
                        if pick_pre_time is None:
                            return [[[observe_pre, current_order_state, current_order_num], None,
                                     [observe, current_order_state, current_order_num], None, current_time],
                                    [None, None], None], None, None, None, None, 0, log
            ...
            if arrive_time > observe[6]:  # add overtime punishment for pre-booked order
                reward_pre = - reward_parameter_list[7]
                # reward_pre = 0
                if current_time > observe[6]:
                    reward_pre -= reward_parameter_list[8]
            else:
                reward_pre = 0
            ...
            if order_pre_type == 0: # if the pre-booked order allows pooling
                ...
                if current_order_num != 0:
                    next_finish_index = np.argmin(current_order_state[:current_order_num, 2])
                    _, _, pick_pre_time_new, _ = TSP_route((current_order_state[next_finish_index,0], current_order_state[next_finish_index,1]), [(observe[2], observe[3])])
                    if pick_pre_time is None:
                        return [[[observe_pre, current_order_state, current_order_num], None,
                                 [observe, current_order_state, current_order_num], None, current_time],
                                [None, None], None], None, None, None, None, 0, log
                    pick_pre_time_new = pick_pre_time_new[0]
                    arrive_time_new = pick_pre_time_new + current_order_state[next_finish_index, 2] + current_time
                else:
                    arrive_time_new = 0

                if (arrive_time_new >= observe[6]) or ( (current_order_num==0) and (advance_time + arrive_time >= observe[6]) ): # start to pick up the pre-booked order
                    assign_state = 1
                    ...
                    pickup_time3 = max(pick_pre_time, observe[6] - current_time)
                    new_total_travel_time[:-1] = new_total_travel_time[:-1] + pickup_time3

                    pick_pre_time2 = max(arrive_time - observe[6], 0) # the waiting time of pre-booked customer
                    ...
                    log["detour"] = new_time[-1] - direct_time[0] + np.sum(new_time[:-1]) - np.sum(current_order_state[:, 2]) + pickup_time3 * current_order_num


                    return [[[observe_pre, current_order_state, current_order_num], order_pre, [observe, current_order_state, current_order_num], None, current_time], [reward_pre, reward], pickup_time3], new_route, new_route_time, new_time, new_total_travel_time, assign_state, log  # in this circumstance, the assignment must be None
                else:
                    reward_pre = 0
                ...
                if current_order_num == 0: # if no unfinished order
                    ...
                    pick_pre_time = pick_pre_time[0]
                    arrive_time = pick_pre_time + current_time
                    if arrive_time + advance_time >= observe[6]: # start to pick up the pre-booked order
                        assign_state = 1
                        if arrive_time > observe[6]:
                            reward_pre = - reward_parameter_list[1] * (arrive_time - observe[6])
                            # reward_pre *= reward_parameter_list[5]
                        ...
                        if new_route is None:
                            return [[[observe_pre, current_order_state, current_order_num], None,
                                     [observe, current_order_state, current_order_num], None, current_time],
                                    [None, None], None], None, None, None, None, 0, log
                        pick_pre_time2 = max(arrive_time - observe[6], 0)
                        # no previous order
...
def obs_pre(observe, order_pre, current_order_state, current_order_num, time_threshold, reward_func, reward_parameter_list, current_time):
    assign_state = 0 # 0: no action, 1: pick up pre-booked order, 2: pick up on-demand order
    reward_pre = 0
    current_order_num = int(current_order_num)
...
def obs(observe, new_orders_state, current_order_state, current_order_num, time_threshold, reward_func, reward_parameter_list, current_time):
    assign_state = 0 # 0: no action, 1: pick up pre-booked order, 2: pick up on-demand order
    reward_pre, reward = 0, 0
    current_order_num = int(current_order_num)
    ...
    order_pre = observe[2:8]

    # 0. whether to start pick up pre-booked order
    if order_pre[0]!=0: # has pre-booked order
        order_pre_type = observe[7]

        if worker_type != 1: # if the worker is available
    ...
    if worker_type == 0 : # assign on-demand order (worker_type must be 0)
        plat, plon, dlat, dlon, appear_time, type, _ = new_orders_state
        waiting_time = current_time - appear_time
