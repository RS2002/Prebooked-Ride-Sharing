...
def main():
    ...
    print("Pre-booked Start Time: {:} , End Time: {:} , Advance Time {:}".format(prebook_start, prebook_end,
                                                                                 advance_time))
    ...
    for t in pbar:
        ...
        #                                     None, 0)
        order_pre = worker.observe(True, demand.current_demand_pre, t,
                                            None, 0)
        if order_pre is not None:
            q_value = platform.obs(worker.observe_space, order_pre,  worker.current_orders,
            worker.current_order_num, args.order_max_wait_time, reward_func, args.reward_parameter2, t, pre=True)
        ...
        order = worker.observe(False, demand.current_demand, t, None, 0)
        if order is not None:
            q_value = platform.obs(worker.observe_space, order, worker.current_orders,
                                   worker.current_order_num, args.order_max_wait_time, reward_func,
                                   args.reward_parameter2, t, pre=False)
            q_value[worker.observe_space[:, 10] == 1] = -1e8
            for j in range(q_value.shape[1]):
                if order[j, -2] == 1:
                    q_value[worker.current_order_num != 0, j] = -1e8
